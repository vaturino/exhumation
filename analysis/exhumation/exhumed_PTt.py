    ...
    for ind_m, m in tqdm(enumerate(configs['models'])): 
        ...
        for i in range(0,npa):
            ...
            if p.P.max() < 3.:
                if (p.P > 0.1).any():
                    exh[i] = i
                    a1[1].plot(p["T"] - 273, p["P"], linewidth = 0.5, c = line_colors[i])
                    a1[1].set_xlabel("T ($^\circ$C)")
                    a1[1].set_ylabel("P (GPa)")
                    a1[1].set_ylim(0,4.5)
                    a1[1].set_xlim(0,1000)
                    a1[1].set_title("Particles with max P < 4.5 GPa")

            else:
                    a1[0].plot(p["T"] - 273, p["P"], linewidth = 0.5, c = line_colors[i])
                    a1[0].set_xlabel("T ($^\circ$C)")
                    a1[0].set_ylabel("P (GPa)")
                    a1[0].set_ylim(0,4.5)
                    a1[0].set_xlim(0,1000)
