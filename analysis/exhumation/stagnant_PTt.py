...
def main():
    ...
    for ind_m, m in tqdm(enumerate(configs['models'])): 
        ...
        eloc = f"{plot_loc}/stagnant"
        if not os.path.exists(eloc):
            os.mkdir(eloc)

        exhloc = f"{txt_loc}/stagnant"
        ...
        print("num of potentially stagnant = ", len(exh), " particles")
        ts = int(len(time_array))
        P = np.zeros(ts-1)
        T = np.zeros(ts-1)
        P[:]=np.nan
        ...
        threshold = .09
        filename = f"{txt_loc}/stagnant_maxPT.txt"
        ...
        for ind_j, j in tqdm(enumerate(exh[::freq])):
            ...
            if (len(e.P) >= 10 and e.depth.iloc[-1] >= (e.depth.max() - 150.)):
                if ((e.depth.iat[-1] - e.depth.min()) >= 0. and (e.depth.iat[-1] - e.depth.min()) <= 2.):
                # if e.depth.min() > (e.depth.max() - 30.):
                 ...
                 # and abs(trench[-1] - a.x.iloc[-1]) <= 50.e3:
                    if e.P.iloc[0]<0.2:
                        ...
                        exname = f"{exhloc}/stagnant_{int(j)}.txt"
                        expar = open(exname,"w+")
                        ...
                        # count = count+1
        plt.savefig(f"{plot_loc}/stagnant_particles.png", dpi = 1000)     
        plt.close()
        maxx.close()
        
        fig, ax = plt.subplots(1, 3, figsize = (15,5))
        # mycmap = colors.ListedColormap(['magenta', 'pink'])
        maxconds = pd.read_csv(f"{txt_loc}/stagnant_maxPT.txt", sep="\s+")
        ...
        fig.suptitle(f"Total number of stagnant particles = {len(maxconds)} - {(len(maxconds)/npa) * 100  :.1f} %")
        fig.tight_layout()
        plt.savefig(f"{plot_loc}/stagnant_max_PT_conditions.png", dpi = 1000)
        plt.close()
        ...
        print("num of stagnant = ", count, " particles")
