...
def main():
    ...
    grid_res=7.e3; grid_low_res = 30.e3; grid_high_res = 2.5e3
    ...
    for ind_m, m in tqdm(enumerate(configs['models'])):    
        ...
        # for t in tqdm(range(5)):
            ...
            P = griddata((data.loc[:,'Points:0'], data.loc[:,'Points:1']), data.loc[:,'nonadiabatic_pressure'],    (X_low, Y_low), method='cubic')


            ax2=fig.add_subplot(gs[0,0], aspect=1)
            # plot visc and crust contour
            visc_plot = ax2.contourf(X_low/1.e3, (ymax_plot-Y_low)/1.e3, P, cmap=matplotlib.colormaps.get_cmap('RdBu_r'),levels=np.linspace(-4e8,4e8,601),extend='both')
            ax2.contour(X_crust/1.e3, (ymax_plot-Y_crust)/1.e3, comp, levels=[0.5],    linewidths=0.5, colors='black',zorder=2)
            ...
            ax2.quiverkey(vel_vects, 0.15, 0.1, 5, '5 cm/yr', labelpos='W',fontproperties={'size': '7'},color='black',labelcolor='black')
            cbar = plt.colorbar(visc_plot, cax = fig.add_axes([0.35, 0.23, 0.5, 0.0125]), orientation='horizontal',ticks=[-4e8, -2e8, 0, 2e8, 4e8], ticklocation = 'top', format=ticker.FuncFormatter(fmt))
            cbar.ax.tick_params(labelsize=5)
            cbar.set_label("$P_{non-ad}$  [Pa]",size=7.5)
            ax2.annotate(''.join(['t = ',str("%.1f" % (time_array[t,1]/1.e6)),' Myr']), xy=(0.01,-0.5), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=13,color='k') 
