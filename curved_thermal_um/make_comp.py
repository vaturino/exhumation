...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		# flat portion of SP
		if x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick):
			C[ind,2]=1
		...
		if x > (x_gap + x_SP - radius_outer) and x < (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) < radius_outer**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick)**2 and y > (ymax - depth_notch): 
				C[ind,2]= 1
...
f.write("# Columns: x y compsp compop\n")
