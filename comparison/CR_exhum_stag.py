...
def main():
    ...
    for idx, test in enumerate(tests):
        ...
        # # Create the plot
        fig, ax = plt.subplots(4, 2, figsize=(20, 15), gridspec_kw={'height_ratios': [0.35, 1, 1, 1]})
        ...
        if test in models:
            ...
            for ind_m, m in enumerate(models[test]):
                ...
                ax[0, 0].plot(cr["time"]/1e6, cr["conv_rate"], color="grey", label="Convergence rate (cm/yr)", linewidth=3)
                ax[0, 0].axvline(x=35, color="grey", linestyle="--")
                ax[0, 0].label_outer()  # Only show outer labels and tick labels
                # ax[0, 0].legend(loc='upper right')
                ax[0, 0].set_ylim(0, cr["conv_rate"].max()+0.3)
                ax[0, 0].set_xlim(0, 50)

                ax[0, 1].plot(cr["time"]/1e6, cr["conv_rate"], color="grey", label="Convergence rate (cm/yr)", linewidth=3)
                ax[0, 1].axvline(x=35, color="grey", linestyle="--")
                ax[0, 1].label_outer()  # Only show outer labels and tick labels
                # ax[0, 1].legend(loc='upper right')
                ax[0, 1].set_ylim(0, cr["conv_rate"].max()+0.3)
                ...
                ax[ind_m+1, 0].axvline(x=35, color="grey", linestyle="--", label="35 Ma")
                ...
                ax[ind_m+1, 1].axvline(x=35, color="grey", linestyle="--", label="35 Ma")
            ...
            plt.savefig(f"{pltloc}/CR_exhumed_stagnant.eps", dpi=500)
            plt.close()
