...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		if x >= (0.) and x <=(xmax):
			if y <= (ymax + ((c_thick-ymax)/(xmax-c_thick))*(x - c_thick)):
		...
		# flat overriding plate (i.e. uniform age)
			elif y >= (ymax + ((c_thick-ymax)/(xmax-c_thick))*(x - c_thick)):
...
# write to file
f= open("text_files/temp_base_obl.txt","w+")
