...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		if x >= 0 and x <= (c_len) and y >= x-c_len+ymax:
			erf_term=(ymax-y)/(2*np.sqrt(k*age))
			T[ind,2]='%.5f'  %   (Tmax - (Tmax - Tmin)*scipy.special.erfc(erf_term))
		# plate boundary region:
		# elif y <= (x+ymax)*ymax/(c_len +ymax) and x < (c_len + ymax) and y<=ymax:
		elif y <= x-c_len+ymax and x < (c_len + ymax) and y<=ymax:
			# subducting plate age below where the crust is (from other script)
			if y <= (ymax*(1 - (x- c_len)/(ymax))): 
			# if y <= x-c_len+ymax:
				# yp = (x-c_len)*np.sin(-alpha) + (ymax-y)*np.cos(-alpha)
				yp = -(x-c_len)*0.71+ (ymax-y)*0.71
				erf_term=(yp)/(2*np.sqrt(k*age_s))
				T[ind,2]='%.5f'  %   (Tmax - (Tmax - Tmin)*scipy.special.erfc(erf_term))
			# overriding plate age above whether the crust is
			elif y >= (ymax*(1 - (x- c_len)/(ymax))): 
				erf_term=(ymax-y)/(2*np.sqrt(k*age_op))
				T[ind,2]='%.5f'  %   (Tmax - (Tmax - Tmin)*scipy.special.erfc(erf_term))
		# flat overriding plate (i.e. uniform age)
		elif x >= (c_len + ymax) and y<=ymax:
				erf_term=(ymax-y)/(2*np.sqrt(k*age_op))
				T[ind,2]='%.5f'  %   (Tmax - (Tmax - Tmin)*scipy.special.erfc(erf_term))
