...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		if x >= 0 and x <= (c_len + ism*np.sin(alpha/2)*((y-ymax)/lith)) and y <=ymax:
			erf_term=(ymax-y)/(2*np.sqrt(k*age))
			T[ind,2]='%.5f'  %   (Tmax - (Tmax - Tmin)*scipy.special.erfc(erf_term))
		# plate boundary region:
		elif x < x2 + dx +  (ism*np.sin(alpha/2)*((y-ymax)/lith))  and y<=ymax:
			x1 = c_len; 
			y1 = ymax - radius;
		...
		elif x >= x2 + dx +  (ism*lith*np.sin(alpha/2)*((y-ymax)/lith)) and y<=ymax:
