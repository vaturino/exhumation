...
input_plot = ax1.scatter(file[:,0]/1.e3, (zmax-file[:,1])/1.e3, c=abs(file[:,3]),cmap='bwr',vmin=cmin,vmax=cmax,s=0.25,lw=0)
# input_plot = ax1.scatter(file[:,0]/1.e3, (zmax-file[:,1])/1.e3, c=np.sqrt(file[:,2]**2 + file[:,3]**2),cmap='bwr',vmin=cmin,vmax=cmax,s=0.25,lw=0)
