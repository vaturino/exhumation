...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		elif x > (x_gap + x_SP + radius_outer) and x <= (xmax - x_gap) and y >= (ymax -cthick - opthick):
			C[ind,3]= 1
		elif x >= x_gap+x_SP and x <= (x_gap + x_SP + radius_outer) and  y >= ymax - (cthick + opthick)/2*(1 + ((x - x_SP - x_gap)/radius_outer) ):
			C[ind,3]=1
		...
		if x > (x_gap + x_SP - radius_outer) and x < (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) < (radius_outer - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed)**2 and y > (ymax - cutoff): 
				...
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,4]=1
			elif ((x-x1)**2 + (y-y1)**2) >= radius_outer**2 and x < x_gap+x_SP+radius_outer and  y >= ymax - (cthick + opthick)/2:
				C[ind,3]=1
