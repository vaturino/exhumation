...
if file_name == 'temp_base':
    input_plot = ax1.scatter(file[:,0]/1.e3, (zmax-file[:,1])/1.e3, c = file[:,2],cmap='bwr',vmin=cmin,vmax=cmax,s=0.25,lw=0)
else:
    input_plot = ax1.scatter(file[:,0]/1.e3, (zmax-file[:,1])/1.e3, c=file[:,2]+2*file[:,3]+3*file[:,4]+4*file[:,5],cmap='bwr',vmin=cmin,vmax=cmax,s=0.25,lw=0)
...
plt.setp(ax1.spines.values(), lw=0.1, alpha=1)
