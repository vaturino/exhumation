...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		# flat portion of SP
		xa = x_gap + x_SP - radius_outer- wz
		xb = x_gap + x_SP - radius_outer- wz/2
		if x >= (x_gap) and x <= (xa + (xb - xa)*(ymax - y)/(sed + cthick)) and y >= (ymax -cthick-sed) and y <= (ymax -sed):
			C[ind,2]=1
		# overiding plate (OP) - linear geotherm
		elif x >= (x_gap + x_SP) and x <= (xmax - x_gap) and y >= (ymax -cthick):
			C[ind,2]= 1
		elif x >= (x_gap + x_SP) and x <= (xmax - x_gap) and y >= (ymax -opthick) and y <= (ymax -cthick):
			C[ind,3]= 1
		elif x >= (x_gap) and x <= (xa + (xb - xa)*(ymax - y)/(sed + cthick)) and y >= (ymax -sed):
			C[ind,4]=1
		elif x > (xa + (xb - xa)*(ymax - y)/(sed + cthick)) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick-sed):
			C[ind,5]=1
...
f.write("# Columns: x y compsp compop compsed compwz\n")
