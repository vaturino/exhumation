...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		# flat portion of SP
		if x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick-sed) and y <= (ymax -sed + ampl*(np.sin((2*np.pi*x)/(wavel)))):
			C[ind,2]=1
		...
		# overiding plate (OP) - linear geotherm
			...
			C[ind,3]= 1
		elif x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -sed + ampl*(np.sin((2*np.pi*x)/(wavel)))) and y <= (ymax):
			C[ind,4]=1
