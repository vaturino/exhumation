		...
		if x > (x_gap + x_SP - radius_outer) and x < (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) < (radius_outer - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed)**2 and y > (ymax - cutoff): 
				...
				if angle > np.radians(90. - slab_dip):
					...
					C[ind,2]=1
			elif ((x-x1)**2 + (y-y1)**2) >= (radius_outer+ wz_thick)**2 and  y >= (ymax -opcthick):
				C[ind,5]=1
				...
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,4]=1
			elif ((x-x1)**2 + (y-y1)**2) >= (radius_outer+ wz_thick)**2 and  y >= ymax - (opcthick + oplthick):
				C[ind,3]=1
			elif ((x-x1)**2 + (y-y1)**2) > radius_outer**2 and ((x-x1)**2 + (y-y1)**2) <= (radius_outer+ wz_thick)**2 and y >= ymax - (3./2)*(opcthick + oplthick):
				C[ind,7]=1
...
f.write("# Columns: x y compsp compop compsed compopc compcore compwz\n")
for k in range(0,ind):
	f.write("%.4f %.4f %.2f %.2f %.2f %.2f %.2f %.2f\n" % (C[k,0],C[k,1],C[k,2],C[k,3],C[k,4],C[k,5],C[k,6],C[k,7]))
f.close() 
