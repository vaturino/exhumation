...
if file_name == 'comp_base':
    input_plot = ax1.scatter(file[:,0]/1.e3, (zmax-file[:,1])/1.e3, c=file[:,2]+2*file[:,3]+3*file[:,4]+4*file[:,5]+5*file[:,6]+6*file[:,7]+7*file[:,8] + 8*file[:,9]+ 9*file[:,10],cmap='bwr',vmin=cmin,vmax=cmax,s=0.25,lw=0)
else:
