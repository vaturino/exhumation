...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		elif x >= x_gap and x <= (x_gap + x_SP - radius_outer) and y >= (ymax - cthick - sed - gabbro_thick) and y <= (ymax - cthick - sed):
			C[ind,3]=1
		# serpentinite under the gabbro
		elif x >= x_gap and x <= (x_gap + x_SP - radius_outer) and y >= (ymax - cthick - sed - gabbro_thick - serp_thick) and y <= (ymax - cthick - sed - gabbro_thick):
			C[ind,9]=1
		...
		if x > (x_gap + x_SP - radius_outer) and x <= (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) <= (radius_outer-sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - gabbro_thick - sed)**2 and  y > (ymax - wz_cutoff): 
				C[ind,8]= 1
			...
			if ((x-x1)**2 + (y-y1)**2) <= (radius_outer)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - sed)**2 and y > (ymax - opcthick): 
				...
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,5]=1
			elif ((x-x1)**2 + (y-y1)**2) <= (radius_outer - sed - cthick)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed - gabbro_thick)**2 and  y >= (ymax - eclogite_cutoff): 
				C[ind,3]= 0
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,3]=1
			elif ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed - gabbro_thick - serp_thick)**2 and ((x-x1)**2 + (y-y1)**2) <= (radius_outer - cthick - sed - gabbro_thick)**2 and  y >= (ymax - eclogite_cutoff):
				C[ind,9]= 0
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
...
f.write("# Columns: x y compsp compgabbro compop compsed compopc compcore compwz compserp compecl\n")
for k in range(0,ind):
	f.write("%.4f %.4f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n" % (C[k,0],C[k,1],C[k,2],C[k,3],C[k,4],C[k,5],C[k,6],C[k,7], C[k,8], C[k,9], C[k,10]))
f.close() 
