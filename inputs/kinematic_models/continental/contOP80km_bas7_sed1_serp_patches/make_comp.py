...
radius_serp = 183.125e3
center_C1 = (0.e3, radius_serp - 5.e3)

# Parameters for spacing between centers of circles
# spacing = 15.e3 + 42.650e3 + np.sqrt(radius**2 - center_C1[1]**2)
spacing = 57.5e3 + np.sqrt(radius_serp**2 - center_C1[1]**2)
n = 30  # Number of circles
center = np.zeros((2, n))

for l in range(n):
    center[:,l] = (l * spacing+ center_C1[0], center_C1[1])
...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		# serpentinite
		for p in range(n):
			if x >= x_gap and x <= (x_gap + x_SP) and y <= (ymax - cthick - sed) and ((ymax - cthick - sed - y + center[1, p])**2 + (x - center[0, p])**2 <= radius_serp**2):
				C[ind,9]=1
		...
		if x > (x_gap + x_SP - radius_outer) and x <= (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) <= (radius_outer)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick)**2 and  y > (ymax - wz_cutoff): 
				...
				if angle > np.radians(90. - slab_dip):
					...
					C[ind,7]=0
					C[ind,9] = 0
			if ((x-x1)**2 + (y-y1)**2) <= (radius_outer - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick)**2 and y > (ymax - eclogite_cutoff): 
				C[ind,2]= 0
				C[ind,9] = 0
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
			...
			if ((x-x1)**2 + (y-y1)**2) <= (radius_outer)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - sed)**2 and y > (ymax - opcthick): 
				...
				if angle > np.radians(90. - slab_dip):
					...
					C[ind,4]=1
					C[ind,9] = 0
			elif ((x-x1)**2 + (y-y1)**2) >= (radius_outer-sed)**2 and  y >= (ymax -opcthick):
...
f.write("# Columns: x y compsp compop compsed compopc compcore compwz compecl compserp\n")
