...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		if x > (x_gap + x_SP - radius_outer) and x < (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) < (radius_outer - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed)**2 and y > (ymax - wz_cutoff): 
				C[ind,7]= 1
				...
				C[ind,5]=1
			elif ((x-x1)**2 + (y-y1)**2) < (radius_outer)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - sed)**2 and y > (ymax - wz_cutoff): 
				C[ind,7]= 1
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
