...
x_gap = 400.e3;         # distance between plate edges and domain sides
...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		# curved portion of strong core 
			...
			z1 = ymax - radius_outer;
			if ((x-x1)**2 + (y-z1)**2) < radius_outer**2 and ((x-x1)**2 + (y-z1)**2) <= (radius_outer-y_coreSP)**2 and ((x-x1)**2 + (y-z1)**2) >= (radius_outer-y_coreSP - core_thick)**2 and y > (ymax - depth_notch):
				C[ind,6]= 0
...
f.write("# Columns: x y compsp compop compbox compopc compcore\n")
