...
y3 = 0
x4 = x2
y4 = zmax + radius*np.cos(alpha) - radius - c_thick/np.cos(alpha)
x5 = x3 - c_thick
y5 = y3
...
for j in tqdm(range(znum + 1)): 
        for i in range(xnum + 1):
            ...
            if x > 0 and x <= (c_len) and z > (zmax - c_thick):
                C[ind,2]=1
            # curved portion of crust (i.e. plate boundary)
            elif x > (c_len) and x < (c_len + radius*np.sin(alpha)):
                x1 = c_len ; 
                z1 = zmax - radius
                if ((x-x1)**2 + (z-z1)**2) < radius**2 and ((x-x1)**2 + (z-z1)**2) >= (radius-c_thick)**2: 
                    C[ind,2]=1
            elif x > (c_len + radius*np.sin(alpha)) and z < y2 + ((y3-y2)/(x3-x2))*(x - x2) and z > y4 + ((y5-y4)/(x5-x4))*(x - x4) and z >= zmax - 100.e3:
                C[ind,2]=1
