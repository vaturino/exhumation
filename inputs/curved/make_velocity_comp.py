...
thick = (dist-4.e3)/np.cos(gamma)

# ip = np.sqrt((x3-x2)**2 + (y3 - y2)**2)
# delta = m.asin((200.e3 - radius*np.sin(alpha))/ip)
...
for j in tqdm(range(znum + 1)): 
        for i in range(xnum + 1):
            ...
            #         beta = np.arccos((x-c_len)/(radius-3.e3))
            #         r = np.sqrt((x-x1)**2 + (z-y1)**2)
            #         vr = v*(1 + (np.cos(alpha) -1)*(radius - r)/lith)
            #         C[ind,2]=vr*np.sin(beta)
            #         C[ind,3]=-vr*np.cos(beta)
            # elif x > (c_len ) and z <= y1 + (y2 - y1)*((x - x1)/(x2 - x1)):
            #     if z < y2 + ((y3-y2)/(x3-x2))*(x - x2) and x >= x2 - thick  +(x3 - x2)/(y3 - y2)*(z-y2):
            #         C[ind,2]=v*np.sin(alpha)
            #         C[ind,3]=-v*np.cos(alpha)

            if x > c_len: # and x < (c_len + radius*np.sin(alpha)): # and z >= y1 + (y2 - y1)*((x - x1)/(x2 - x1)):
                if ((x-x1)**2 + (z-y1)**2) < radius**2: # and ((x-x1)**2 + (z-y1 +lith)**2) >= (radius)**2:
                    beta = np.arccos((x-c_len)/(radius-3.e3))
                    r = np.sqrt((x-x1)**2 + (z-y1)**2)
                    vr = v*r/radius
                    C[ind,2]=vr*np.sin(beta)
